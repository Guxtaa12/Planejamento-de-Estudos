...
async function registrarUsuario(event) {
    ...
    // Pega os valores usando os IDs do seu HTML
    const emailOuUsuario = document.getElementById('registrar-usuario').value;
    const senhaDigitada = document.getElementById('registrar-senha').value;
    const mensagemTexto = document.getElementById('register-message'); // Aquele seu <p> vazio

    // Monta o "pacote" para o Python
    const dadosUsuario = {
        nome: emailOuUsuario, // Mandando o usuário como nome
        email: emailOuUsuario, // Mandando o usuário como email
        senha: senhaDigitada
    };
    ...
    try {
        ...
        if (resposta.ok) {
            ...
            mensagemTexto.innerText = 'Conta criada com sucesso! Redirecionando...';
            
            // Espera 1 segundo e meio e joga para a tela de login
            setTimeout(() => {
                window.location.href = 'login.html'; 
            }, 1500);
        } else {
            // Mostra erro em vermelho (ex: Email já existe)
            mensagemTexto.style.color = '#dc3545';
            mensagemTexto.innerText = 'Erro: ' + resultado.erro;
        }
    } catch (erro) {
        console.error('Erro:', erro);
        mensagemTexto.style.color = '#dc3545';
        mensagemTexto.innerText = 'Erro ao conectar com o servidor.';
    }
}

// Conecta a função ao seu formulário
...
async function fazerLogin(event) {
    ...
    const emailOuUsuario = document.getElementById('login-usuario').value;
    const senhaDigitada = document.getElementById('login-senha').value;
    // Puxando o ID exato que você colocou no seu HTML
    ...
    try {
        ...
        if (resposta.ok) {
            mensagemTexto.style.color = '#28a745'; // Verde para sucesso
            mensagemTexto.innerText = 'Bem-vindo, ' + resultado.nome + '! Entrando...';
            
            // Joga para a home após 1 segundo
            setTimeout(() => {
                window.location.href = 'home.html'; 
            }, 1000);
        } else {
            mensagemTexto.style.color = '#dc3545'; // Vermelho para erro
            mensagemTexto.innerText = 'Erro: Usuário ou senha incorretos!';
        }
    } catch (erro) {
        console.error('Erro:', erro);
        mensagemTexto.style.color = '#dc3545';
